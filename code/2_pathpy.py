...
In the introductory lecture we have seen that higher-order modelling, visualisation, and analysis techniques are useful to analyze **temporal network data** that provide us with **statistics of causal paths**. But how can we apply higher-order network analytics to such such data in practice?
...
`pathpy` is pure `python` code with no platform-specific dependencies. It only depends on `numpy` and `scipy`, which come with `Anaconda`, so it should be easy to install. In principle installing the latest 2.0 version of `pathpy` should be as easy as running

```
pip install pathpy2
```

on the terminal. In any case, you can find more detailed setup instructions on the [tutorial website](https://ingoscholtes.github.io/eurocss2019-tutorial/setup).
...
<span style="color:red">**TODO**: Save your visualisation to a file `2_simple_network.html`. Reuse the visualisation parameters from above.</span>
""")
